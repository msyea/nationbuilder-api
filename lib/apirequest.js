...
function parsePath(path, params) {
  ...
  return utils.parseString(path, escapedParams);
}

/**
 * Create and send request to NationBuilder API
...
function createAPIRequest(parameters, callback) {
  ...
  params = utils.extend({}, // New base object
    parameters.context.nationbuilder._options.params, // Global params
  ...
  var authClient = params.auth ||
    ...
    parameters.context.nationbuilder._options.auth;

  var defaultMime = typeof media.body === 'string' ? 'text/plain' :
  ...
  if (parameters.mediaUrl && media.body) {
    ...
    if (resource) {
      ...
      options.multipart = [{
        ...
        'Content-Type': media.mimeType || resource && resource.mimeType ||
          defaultMime,
  ...
  options = utils.extend({},
    parameters.context.nationbuilder._options,
